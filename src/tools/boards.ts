...
import type { TrelloBoard, TrelloLabel, TrelloMember } from "../types.js";
    ...
    async ({ include_closed }) => {
      try {
        ...
        const boards = await client.get<TrelloBoard[]>(
          "/members/me/boards",
          { fields: "name,url,shortUrl,closed,desc" }
        );
...
Returns:
  Board object with id, name, description, url, and lists array.`,
    ...
    async ({ board_id }) => {
      try {
        const client = getTrelloClient();
        const board = await client.get<TrelloBoard>(
          `/boards/${board_id}`,
          { fields: "name,desc,url,shortUrl,closed", lists: "open" }
        );
  ...
  Updated board confirmation.`,
    ...
    async ({ board_id, name, desc, closed }) => {
      try {
        ...
        const board = await client.put<TrelloBoard>(`/boards/${board_id}`, updates);
