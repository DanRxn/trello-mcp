...
Returns:
  Created list with id and name.`,
        ...
        try {
            ...
            const list = await client.post("/lists", {
                idBoard: board_id,
                name,
...
Returns:
  Updated list confirmation.`,
        ...
        try {
            ...
            const list = await client.put(`/lists/${list_id}`, updates);
            return {
                content: [
...
Returns:
  Array of cards with id, name, description, due date, labels, and members.`,
        ...
        try {
            const client = getTrelloClient();
            const cards = await client.get(`/lists/${list_id}/cards`, {
    ...
    server.registerTool("trello_archive_all_cards", {
...
Returns:
  Confirmation message.`,
    ...
    server.registerTool("trello_move_all_cards", {
...
Returns:
  Confirmation message.`,
    ...
    // --- Move List to Board ---
    server.registerTool("trello_move_list", {
...
Returns:
  Confirmation message.`,
        ...
        try {
            const client = getTrelloClient();
            const list = await client.put(`/lists/${list_id}/idBoard`, {
                value: board_id,
            });
            return {
                content: [
