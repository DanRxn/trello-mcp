...
Returns:
  Card details with name, description, due date, labels, checklists (with items), members, and attachments.`,
        ...
        try {
            ...
            const card = await client.get(`/cards/${card_id}`, {
  ...
  Updated card confirmation.`,
        ...
        try {
            ...
            if (desc !== undefined)
                updates.desc = desc;
            if (due !== undefined)
                updates.due = due === "null" ? "" : due;
            if (due_complete !== undefined)
                updates.dueComplete = String(due_complete);
            if (closed !== undefined)
            ...
            const client = getTrelloClient();
            const card = await client.put(`/cards/${card_id}`, updates);
            return {
                content: [
                    {
                        type: "text",
    ...
    // --- Delete Card ---
    server.registerTool("trello_delete_card", {
...
Returns:
  Confirmation message.`,
    ...
    // --- Search Cards ---
    server.registerTool("trello_search_cards", {
...
Returns:
  Array of matching cards.`,
        ...
        try {
            ...
            let output = JSON.stringify({ query, count: result.length, cards: result }, null, 2);
...
Returns:
  Confirmation message.`,
    ...
    // --- Get Card Comments ---
    server.registerTool("trello_get_card_comments", {
...
Returns:
  Array of comments with author and text.`,
        ...
        try {
            const client = getTrelloClient();
            const actions = await client.get(`/cards/${card_id}/actions`, { filter: "commentCard" });
    ...
    // --- Update Comment ---
    server.registerTool("trello_update_comment", {
...
Returns:
  Confirmation message.`,
    ...
    // --- Delete Comment ---
    server.registerTool("trello_delete_comment", {
...
Returns:
  Confirmation message.`,
    ...
    // --- Add Label to Card ---
    server.registerTool("trello_add_card_label", {
...
Returns:
  Confirmation message.`,
    ...
    server.registerTool("trello_remove_card_label", {
...
Returns:
  Confirmation message.`,
    ...
    server.registerTool("trello_assign_member", {
...
Returns:
  Confirmation message.`,
    ...
    server.registerTool("trello_remove_member", {
...
Returns:
  Created attachment info.`,
        ...
        try {
            const client = getTrelloClient();
            const attachment = await client.post(`/cards/${card_id}/attachments`, { url, name });
            return {
                content: [
                    {
                        type: "text",
    ...
    // --- Get Attachments ---
    server.registerTool("trello_get_attachments", {
...
Returns:
  Array of attachments.`,
        ...
        try {
            const client = getTrelloClient();
            const attachments = await client.get(`/cards/${card_id}/attachments`);
    ...
    // --- Delete Attachment ---
    server.registerTool("trello_delete_attachment", {
...
Returns:
  New card info.`,
    ...
    }, async ({ card_id, list_id, name, keep_attachments, keep_checklists, keep_comments, keep_labels, keep_members, keep_stickers, }) => {
        try {
            const client = getTrelloClient();
            // Build keepFromSource string
            const keep = [];
            if (keep_attachments)
                keep.push("attachments");
            if (keep_checklists)
                keep.push("checklists");
            if (keep_comments)
                keep.push("comments");
            if (keep_labels !== false)
                keep.push("labels"); // Default true
            if (keep_members)
                keep.push("members");
            if (keep_stickers)
                keep.push("stickers");
