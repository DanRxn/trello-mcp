...
export function registerMemberTools(server) {
    // --- Get Current User ---
    server.registerTool("trello_get_me", {
...
Returns:
  User info including id, username, fullName, and email.`,
        ...
        try {
            ...
            const member = await client.get("/members/me", { fields: "id,username,fullName,initials,avatarUrl,email" });
            const result = {
                id: member.id,
    ...
    // --- Get Member ---
    server.registerTool("trello_get_member", {
...
Returns:
  Member info including id, username, and fullName.`,
        ...
        try {
            const client = getTrelloClient();
            const member = await client.get(`/members/${member_id}`, { fields: "id,username,fullName,initials,avatarUrl" });
            const result = {
                id: member.id,
                username: member.username,
                fullName: member.fullName,
    ...
    // --- Get Member's Boards ---
    server.registerTool("trello_get_member_boards", {
...
Returns:
  Array of boards with id, name, and url.`,
        ...
        try {
            ...
            const boards = await client.get(`/members/${memberId}/boards`, { fields: "name,url,shortUrl,closed" });
            const result = boards
                .filter((b) => !b.closed)
                .map((b) => ({
                id: b.id,
    ...
    // --- Get Member's Cards ---
    server.registerTool("trello_get_member_cards", {
...
Returns:
  Array of cards with id, name, board, and list info.`,
        ...
        try {
            const client = getTrelloClient();
            const memberId = member_id || "me";
            const cards = await client.get(`/members/${memberId}/cards`, {
    ...
    // --- Search Members ---
    server.registerTool("trello_search_members", {
  ...
  Array of matching members.`,
        ...
        try {
            ...
            if (board_id) {
                ...
                const allMembers = await client.get(`/boards/${board_id}/members`);
                const lowerQuery = query.toLowerCase();
                members = allMembers.filter((m) => m.fullName.toLowerCase().includes(lowerQuery) ||
